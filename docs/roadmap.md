...
本文件是后续 Vibe Coding 的功能范围控制器。当前状态：**Phase 0 的 Day 1 Bootstrap 已交付；Phase 1 正在开发；Steps 1.1–1.7 已完成，Step 1.8 已实现并通过 Provider 契约测试，但因当前环境未配置 `OPENAI_API_KEY`，最小真实调用仍待验证。完整 Python Coding Agent MVP 仍为 NOT IMPLEMENTED。**
...
- **Status**：Current / In Progress；Steps 1.1–1.7 已完成，Step 1.8 已实现并等待最小真实调用验证，完整 MVP 仍为 NOT IMPLEMENTED。
...
| 1.6 Apply Patch Tool | `apply_patch` 只修改受控 Repository，返回真实 Diff/错误 | 正常修改、冲突/无效 Patch、越界拒绝测试 | Completed |
| 1.7 Run Test Tool | 白名单测试命令、timeout、stdout/stderr、exit code | PASS、FAIL、timeout、非法命令均有真实测试 | Completed |
| **1.8 One Real LLM Provider** | 只接一个真实 Provider 适配到已有核心模型和 Loop | Provider 契约测试 + 一次最小真实调用；Secret 不入日志 | **Current / Implemented / Awaiting Live Validation** |
...
**当前 Step 锁为 Phase 1.8**。本 Step 在 `services/agent_runtime/app/providers/openai_chat.py` 中实现同步 `OpenAIChatModel`，作为现有 `ChatModel` Protocol 的唯一真实 Provider 适配器。它使用标准库向固定的 OpenAI Chat Completions HTTPS 地址发送请求，不引入 SDK 或 Provider 框架；`OPENAI_API_KEY` 仅从构造参数或环境读取，不进入 `repr`，HTTP / 网络错误也会脱敏。适配器将内部 system / user / assistant / tool 消息和 Registry Schema 转为 OpenAI 格式，把完整 `ToolResult` JSON 作为 Observation 回传，并将返回的 function tool calls 解析、校验为内部 `ToolCall`。响应体有 2 MB 默认上限，非 JSON、空 choices、错误 role、非法工具参数和网络失败都显式报错。新增 10 个 Provider 契约测试，连同 Steps 1.1–1.7 共 80 个标准库单元测试通过；`services/agent_runtime/live_openai_provider.py` 提供最小实调入口。当前环境没有 `OPENAI_API_KEY`，因此实调命令仅验证到安全失败（exit 1），本 Step 保持 Awaiting Live Validation，不自动推进 Phase 1.9。受控 Bug-Fix E2E、Provider retry / async / streaming、Responses API、多 Provider 和完整 Schema 语义校验仍未实现。
