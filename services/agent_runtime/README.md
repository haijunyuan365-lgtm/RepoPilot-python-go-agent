...
**Status: PARTIALLY IMPLEMENTED。** Steps 1.1–1.7 已完成；Phase 1.8 OpenAI Provider 已实现并通过契约测试，但最小真实 API 调用因当前环境没有 `OPENAI_API_KEY` 仍待验证；完整 Python Coding Agent MVP 仍为 NOT IMPLEMENTED。
...
Phase 1.6 在 `app/tools/apply_patch.py` 中提供严格的单文件 unified diff `apply_patch`。工具只修改固定 Repository 内既有 UTF-8 普通文本文件，拒绝绝对/穿越/受保护路径、链接写入、新增/删除/重命名和多文件 Patch。所有 header、hunk 计数、顺序、上下文及换行语义先在内存校验和应用，成功后才通过同目录临时文件与 `os.replace` 原子替换；返回值是根据实际前后内容重新生成的 Diff。Patch、目标文件和输出都有字符上限，支持 CRLF 与无末尾换行。`tests/test_apply_patch_tool.py` 使用真实临时文件系统覆盖 10 个成功、冲突、格式、安全、文本和限制场景。连同前五步共有 60 个标准库测试；该 Step 已完成。

Phase 1.7 在 `app/tools/run_test.py` 中提供白名单 `run_test`。构造器将模型可见的 `test_name` 映射到固定 argv，并在初始化时冻结可执行文件绝对路径；执行时固定 Repository cwd、使用最小环境与 `shell=False`，不接受模型提供的命令或额外参数。stdout / stderr 被并发排空但只保留有界前缀；JSON Observation 保存命令、exit code、timeout、双流内容及截断状态。exit code 0 才成功，非零退出和 timeout 都返回包含真实执行证据的失败 `ToolResult`。`tests/test_run_test_tool.py` 使用真实临时 Repository 和子进程覆盖 10 个 PASS、FAIL、timeout、白名单、注入、输出和配置边界；连同前六步共有 70 个标准库测试。该 Step 已完成。

Phase 1.8 在 `app/providers/openai_chat.py` 中提供同步 `OpenAIChatModel`。它把内部 Message、Tool Schema、ToolCall 与 ToolResult Observation 双向转换为 OpenAI Chat Completions function-calling 协议，使用固定 HTTPS 端点、环境 API Key、请求超时和有界响应；错误 role、空 choices、非 JSON、非法工具 arguments、HTTP 和网络错误都显式失败，且 `repr` / 错误信息不泄露 Key。`tests/test_openai_provider.py` 覆盖 10 个配置、协议映射、两轮 Loop 集成、响应边界与脱敏场景；全量标准库测试共 80 个。`live_openai_provider.py` 是最小真实 API 调用入口，当前因没有 `OPENAI_API_KEY` 尚未通过实调，所以 Current Step 仍为 1.8 / Awaiting Live Validation。

Phase 1 后续只剩在专门准备的受控测试 Repository 中验证 Bug Fix E2E。多文件补丁事务、文件新增/删除/重命名、完整 Schema 参数语义校验、Provider retry、async/streaming、Responses API、多 Provider、Loop / Task 级可靠取消、Sandbox 级进程/资源/网络/文件系统隔离和 Bug-Fix E2E 均未实现。

Phase 1–4 本地受控工具是有限过渡；Phase 5 执行职责交给 Go Tool Gateway / Sandbox，Python 保留选择和调用。Phase 8 前 HIGH 工具禁用。范围见 [roadmap](../../docs/roadmap.md)，执行约束见 [AGENTS](../../AGENTS.md)。当前没有 pyproject.toml、外部依赖或常驻服务入口。最小 Provider 实调命令（会产生真实 API 调用和费用）为：先设置 `OPENAI_API_KEY`，再在本目录运行 `python -B live_openai_provider.py --model <可用模型 ID>`。
