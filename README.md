...
**当前已完成治理基线，以及 Phase 1.1–1.3 的核心模型、Tool Registry 和最小 Agent Loop；真实工具、LLM Provider 与平台服务仍未实现。**
...
以上长期架构整体尚未实现；当前只有 Phase 1 的模型、Tool Registry 契约和使用测试替身验证的最小 Agent Loop。详见 [architecture](docs/architecture.md) 和 [八项 ADR](docs/decisions.md)。
...
| 1 | Python Coding Agent MVP | In Progress；Steps 1.1–1.3 已实现，完整 MVP 仍为 NOT IMPLEMENTED |
...
- **Current Step**：Phase 1.3 — Minimal Agent Loop，已实现并通过本地测试，等待用户验收；不会自动推进到 Step 1.4。
- **Implemented**：项目治理基线；Provider 无关的 Message、ToolCall、ToolResult、AgentState、AgentResult；工具调用与结果关联校验；Tool 定义及 Registry；同步 ChatModel 契约；透明 Agent Loop；Tool Call 顺序执行、Observation 回填、自然成功、显式失败和迭代耗尽语义；标准库单元测试。
- **Planned / NOT IMPLEMENTED**：真实 Tools、完整 JSON Schema 参数语义校验、async/streaming、超时/取消、真实 LLM Provider、完整 Python MVP，以及 Go Control Plane、RabbitMQ、Redis、PostgreSQL、pgvector、Tool Gateway、SSE、Sandbox、Code RAG、Memory、Approval、系统化 Eval 和 OpenTelemetry。
- **Validation**：在 Python 3.10.9 使用 `python -B -m unittest discover -s tests -v` 运行 31 个模型、Registry 与 Loop 测试并通过。当前 Loop 仅通过 fake ChatModel / fake Tool 验证，没有真实 Provider、真实仓库工具或服务启动入口。
...
当前 Step 1.3 等待用户验收。验收确认后，下一推荐 Step 是 Phase 1.4 Safe Read Tools；本次未实现该 Step。
