...
以上架构尚未实现。详见 [architecture](docs/architecture.md) 和 [八项 ADR](docs/decisions.md)。
...
| 1 | Python Coding Agent MVP | Preparation，NOT IMPLEMENTED |
...
- **Current Step**：Phase 1.1 — Core Runtime Data Models，已实现并通过本地测试，等待用户验收；Step 锁尚未推进。
- **Implemented**：项目治理基线；Provider 无关的 Message、ToolCall、ToolResult、AgentState、AgentResult；工具调用与结果关联校验；成功/失败/迭代耗尽语义；标准库单元测试。
- **Planned / NOT IMPLEMENTED**：Tool Registry、Agent Loop、真实 Tools、LLM Provider、完整 Python MVP，以及 Go Control Plane、RabbitMQ、Redis、PostgreSQL、pgvector、Tool Gateway、SSE、Sandbox、Code RAG、Memory、Approval、系统化 Eval 和 OpenTelemetry。
- **Validation**：在 Python 3.10.9 使用 `python -B -m unittest discover -s tests -v` 运行 17 个模型测试并通过。当前没有可运行 Agent、Provider 调用或服务启动入口。
- **Git**：Day 1 未初始化 Git、未创建提交/标签或远端；此骨架可在后续纳入独立仓库。
...
当前 Step 1.1 等待用户验收。验收确认后，下一推荐 Step 是 Phase 1.2 Tool Registry Contract；本次未实现该 Step。
